...
export default class Group extends BaseGroup {
    ...
    async getTextSchedule(date = new Date(), opts:{showDate?: boolean} = {}) {
        ...
        return `${days[day]} / ${week ? "Чётная" : "Нечётная"} неделя` + (opts.showDate ? ` / ${date.stringDate()}` : "") + `` + (!text ? "\nПар нет! Передохни:з" : text);
    }
    ...
    async getTextFullSchedule(week: boolean, startDate: Date) {
        ...
        if(schedule == null || schedule == undefined) return "Во время получения расписания произошла ошибка!\nВозможно стоит обратиться в [поддержку|https://t.me/Elektroplayer]";
    ...
    async getTextExams() {
        ...
        if(!resp || !resp.isok) return "Во время получения расписания произошла ошибка!\nВозможно стоит обратиться в [поддержку|https://t.me/Elektroplayer]";
        if(!resp.data.length) return `У меня нет расписания экзаменов для твоей группы...`;

        let examsText = resp.data.reduce((acc, x) => acc + `${x.time_sd.substring(0,x.time_sd.length-3)} ${x.date_sd} / ${x.disc.disc_name}\n  Преподаватель: ${x.teacher}\n  Аудитория: ${x.classroom}\n\n`, "");
